...
class Play {
    ...
    static async endTurn(game) {
        try {
            ...
            await pool.query(`Update user_game set ug_state_id = $1 where ug_id = $2`,
                [1,game.player.id]); 
            ...
            await pool.query(`Update game set gm_turn = gm_turn+1 where gm_id = $1`,
                [game.id]);
    ...
    static async drawCard(game) {
        try {
            ...
            if (game.player.state.name != "Playing") 
            {
                return {status: 400, result:{msg: "You cannot end turn since you are not currently on your turn" }}
            }
        
            //Verify if the player already drawn a card
